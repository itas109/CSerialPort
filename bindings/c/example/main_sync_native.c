...
void onReadData(i_handle_t handle, const char *portName, unsigned int readBufferLen, unsigned int* countRead)
{
    if (readBufferLen > 0)
    {
...
int main()
{
    ...
    else
    {
        ...
        for (;;)
        {
            onReadData(handle, portName, CSerialPortGetReadBufferUsedLen(handle), &countRead);
