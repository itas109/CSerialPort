// CommMFCDLLDlg.cpp : 实现文件
//

#include "stdafx.h"
#include "CommMFCDLL.h"
#include "CommMFCDLLDlg.h"
...
CCommMFCDLLDlg::CCommMFCDLLDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CCommMFCDLLDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CCommMFCDLLDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
...
BEGIN_MESSAGE_MAP(CCommMFCDLLDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_OPEN_CLOSE, &CCommMFCDLLDlg::OnBnClickedButtonOpenClose)
	ON_BN_CLICKED(IDC_BUTTON_SEND, &CCommMFCDLLDlg::OnBnClickedButtonSend)
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON_CLEAR, &CCommMFCDLLDlg::OnBnClickedButtonClear)
END_MESSAGE_MAP()


// CCommMFCDLLDlg 消息处理程序

BOOL CCommMFCDLLDlg::OnInitDialog()
{
	...
	m_SerialPort.readReady.connect(this, &CCommMFCDLLDlg::OnReceive);
	
	return TRUE;  // 除非将焦点设置到控件，否则返回 TRUE
}

void CCommMFCDLLDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR CCommMFCDLLDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CCommMFCDLLDlg::OnBnClickedButtonOpenClose()
{
...
void CCommMFCDLLDlg::OnBnClickedButtonSend()
{
	GetDlgItem(IDC_SendEdit)->SetFocus();
	CString temp;
	UpdateData(true);
...
void CCommMFCDLLDlg::OnClose()
{
	m_SerialPort.close();
	CDialog::OnClose();
}

void CCommMFCDLLDlg::OnReceive()
{
	char * str = NULL;
	str = new char[1024];
...
void CCommMFCDLLDlg::OnBnClickedButtonClear()
{
	rx = 0;
	tx = 0;
	m_recvCountCtrl.SetWindowText(CString("0"));
