...
TEST_F(CSerialPortTests, isOpen_2_1)
{
    ...
    EXPECT_TRUE(isOpen == m_serialport.isOpen()) << m_availablePortsInfoVector[0].portName << " function isOpen() isOpen status not equal function open() ";
}
...
TEST_F(CSerialPortTests, open_close_3_1)
{
    ...
    m_serialport.close();
    EXPECT_FALSE(m_serialport.isOpen()) << m_availablePortsInfoVector[0].portName << " close error, error code: " << m_serialport.getLastError();
}
...
TEST_F(CSerialPortTests, open_close_3_2)
{
    m_serialport.init(m_availablePortsInfoVector[0].portName);
    m_serialport.open();
    EXPECT_TRUE(m_serialport.isOpen()) << m_availablePortsInfoVector[0].portName << " open error, error code: " << m_serialport.getLastError();
    m_serialport.close();
    EXPECT_FALSE(m_serialport.isOpen()) << m_availablePortsInfoVector[0].portName << " close error, error code: " << m_serialport.getLastError();
    
    m_serialport.open();
    EXPECT_TRUE(m_serialport.isOpen()) << m_availablePortsInfoVector[0].portName << " open again error, error code: " << m_serialport.getLastError();
    m_serialport.close();
    EXPECT_FALSE(m_serialport.isOpen()) << m_availablePortsInfoVector[0].portName << " close again error, error code: " << m_serialport.getLastError();
}
...
TEST_F(CSerialPortTests, open_close_3_3)
{
    ...
    m_serialport.open();
    EXPECT_TRUE(m_serialport.isOpen()) << m_availablePortsInfoVector[1].portName << " open again error, error code: " << m_serialport.getLastError();
    m_serialport.close();
    EXPECT_FALSE(m_serialport.isOpen()) << m_availablePortsInfoVector[1].portName << " close again error, error code: " << m_serialport.getLastError();
}
