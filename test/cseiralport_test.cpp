...
TEST_F(CSerialPortTests, isContainsVirtualPair_0_1)
{
    ...
    {
        if (0 == m_availablePortsInfoVector[i].portName.compare(portName1) || 0 == m_availablePortsInfoVector[i].portName.compare(portName2))
        {
...
TEST_F(CSerialPortTests, isOpened_2_1)
{
    ...
    EXPECT_TRUE(isOpen == m_serialport.isOpened()) << portName1 << " function isOpened() isOpened status not equal function open() ";
}
...
TEST_F(CSerialPortTests, open_close_3_1)
{
    ...
    m_serialport.close();
    EXPECT_FALSE(m_serialport.isOpened()) << portName1 << " close error, error code: " << m_serialport.getLastError();
}
...
TEST_F(CSerialPortTests, open_close_3_2)
{
    m_serialport.init(portName1);
    m_serialport.open();
    EXPECT_TRUE(m_serialport.isOpened()) << portName1 << " open error, error code: " << m_serialport.getLastError();
    m_serialport.close();
    EXPECT_FALSE(m_serialport.isOpened()) << portName1 << " close error, error code: " << m_serialport.getLastError();

    m_serialport.open();
    EXPECT_TRUE(m_serialport.isOpened()) << portName1 << " open again error, error code: " << m_serialport.getLastError();
    m_serialport.close();
    EXPECT_FALSE(m_serialport.isOpened()) << portName1 << " close again error, error code: " << m_serialport.getLastError();
}
...
TEST_F(CSerialPortTests, open_close_3_3)
{
    ...
    m_serialport.open();
    EXPECT_TRUE(m_serialport.isOpened()) << portName2 << " open again error, error code: " << m_serialport.getLastError();
    m_serialport.close();
    EXPECT_FALSE(m_serialport.isOpened()) << portName2 << " close again error, error code: " << m_serialport.getLastError();
}
...
#ifdef _WIN32
    strncpy(portName1, "COM40", 256);
    strncpy(portName2, "COM50", 256);
...
#else
    strncpy(portName1, "", 256);
    strncpy(portName2, "", 256);
